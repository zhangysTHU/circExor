...
def _count_kmer(Dataset, k):  # k = 3, 4, 5,  6, 7
    ...
    heptamer = [''.join(n) for n in seven]

    # k == 6:
    six = list(itertools.product(nucleotide, repeat=6))
    hexamer = [''.join(n) for n in six]
    ...
    if k == 34:
        ...
        table_kmer.update(dict.fromkeys(heptamer, 0))
...
def logistic_regression_classification(x_train, y_train, x_test, y_test, output_dir, fold_id="", SEED=42):
    """
    Run binary Logistic Regression with hyperparameter tuning, training, evaluation, and result export.
    ...
    fold_id : fold identifier used to keep outputs separate
...
def SVM_classification(x_train, y_train, x_test, y_test, output_dir, fold_id="", SEED=42):
    """
    Run binary SVM classification with hyperparameter tuning, training, evaluation, and result export.
    """
    fold_str = f" Fold {fold_id}" if fold_id != "" else ""
    print(f"\n*** SVM{fold_str} ***")
...
def lightGBM_classification(x_train, y_train, x_test, y_test, output_dir, fold_id="", SEED=42, n_iter = 100):
    ...
    lgb_rscv = RandomizedSearchCV(lgb_model, lgb_param_dict, n_iter=n_iter,cv = 5,verbose = 0,
                            scoring = "roc_auc",random_state=SEED,n_jobs = 8)
...
def RF_classification(x_train, y_train, x_test, y_test, output_dir, fold_id="", SEED=42, n_iter=100):
    """
    Run binary Random Forest classification with hyperparameter tuning, training, evaluation, and result export.
    """
    fold_str = f" Fold {fold_id}" if fold_id != "" else ""
...
def ngboost_classification_random_search(x_train, y_train, x_test, y_test, output_dir, fold_id="", SEED=42, n_iter=50):
    ...
    ngb_param_dict = {
        ...
        "natural_gradient": [False],      # False only, for k = 34567 mer ,may leads to Singular matrix
        "verbose": [False],
...
# Output dir
output_dir = "kmer67_extra4fold_output"
...
df_kmer_train, df_kmer_train_raw = _count_kmer(train_df, 34567)
df_kmer_val, df_kmer_val_raw = _count_kmer(val_df, 34567)
df_kmer_test, df_kmer_test_raw = _count_kmer(test_df, 34567)

# df_kmer_train.to_csv(os.path.join(output_dir, "train_kmer34567_freq.tsv"), sep='\t')
# df_kmer_train_raw.to_csv(os.path.join(output_dir, "train_kmer34567_rawcount.tsv"), sep='\t')
# df_kmer_val.to_csv(os.path.join(output_dir, "val_kmer34567_freq.tsv"), sep='\t')
# df_kmer_val_raw.to_csv(os.path.join(output_dir, "val_kmer34567_rawcount.tsv"), sep='\t')
# df_kmer_test.to_csv(os.path.join(output_dir, "test_kmer34567_freq.tsv"), sep='\t')
# df_kmer_test_raw.to_csv(os.path.join(output_dir, "test_kmer34567_rawcount.tsv"), sep='\t')
