...
def logistic_regression_classification(x_train, y_train, x_test, y_test, output_dir, SEED=42):
    ...
    # --- 新增：提取并保存五折交叉验证的全部5个模型，并打印必要的评估指标 ---
    from sklearn.model_selection import cross_validate
    import joblib
    
    # 提取最佳参数并配置新模板进行交叉验证拟合（以保留所有各折模型）
...
def SVM_classification(x_train, y_train, x_test, y_test, output_dir, SEED=42):
    ...
    # --- 新增：提取并保存五折交叉验证的全部5个模型，并打印/保存必要的评估指标 ---
    from sklearn.model_selection import cross_validate
    import joblib
    import pandas as pd
...
def catboost_classification_random_search(x_train, y_train, x_test, y_test, output_dir, SEED=42, n_iter=100):
    ...
    cv_template_model.set_params(verbose=False) # 防止在 CV 过程中输出过多日志
...
def ngboost_classification_random_search(x_train, y_train, x_test, y_test, output_dir, SEED=42, n_iter=50):
    ...
    # NGBClassifier 需要显式指定 Dist=k_categorical(2) 才能处理二分类
...
# Output dir
output_dir = "/lulabdata3/huangkeyun/zhangys/RNA_locator/ML_python_scripts/cross_specises/corss_specises_output"
...
# %%
# 去除训练集中所有小鼠源circRNA，验证集和测试集则保留
train_df = train_df[train_df['Species'] != 'Mus musculus']
...
fasta_records = [
    ...
    for _, row in val_test_df.iterrows()
]

# 保存为 fasta 文件
fasta_path = os.path.join(output_dir, "val_test_set_sequences.fasta")
SeqIO.write(fasta_records, fasta_path, "fasta")
print(f"已成功将验证集和测试集序列保存至: {fasta_path}")
